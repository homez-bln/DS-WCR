...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!wcr_verify_csrf(false)) {
        ...
        if ($action === 'ig_save_connection') {
            ...
            // Token nur überschreiben, wenn wirklich neu gesetzt
            if ($token !== '') {
            ...
            if ($r['ok'] && !empty($r['json']['ok'])) {
                ...
                $msg = 'Fehler beim Speichern der Instagram-Verbindung: ' . ($r['err'] ?: 'Unbekannt');
                $msgType = 'error';
            }
        }

        // ── Instagram Feed-Einstellungen speichern ─────────────────────────────────
...
$ig_rest = dsc_curl(DSC_WP_API_BASE . '/instagram');
$ig_rest_posts = 0;
$ig_rest_ok = false;
$ig_rest_info = 'REST-Feed aktuell nicht erreichbar';
$ig_rest_preview = '[]';
if ($ig_rest['ok']) {
    if (is_array($ig_rest['json'])) {
        $ig_rest_ok = true;
        ...
        $ig_rest_preview = json_encode($ig_rest['json'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if ($ig_rest_preview === false) $ig_rest_preview = (string)($ig_rest['json'] ?? '');
    }
} elseif ($ig_rest['err']) {
...
.ig-debug-card{padding:12px;border:1px solid var(--border-light);border-radius:10px;background:var(--bg-subtle);}
...
<div class="ig-block">
  ...
  <form method="POST" id="ig-connection-form">
    <?= wcr_csrf_field() ?>
    <input type="hidden" name="action" value="ig_save_connection">
    <div class="ig-grid">
      <div class="ig-row">
        <label class="ig-label">Access Token</label>
        <span class="ig-sublabel">Meta Graph API — Long-lived token. Leer lassen = bestehenden Token behalten.</span>
    ...
    <div class="ig-footer" style="margin-top:0;padding-top:0;border-top:none;margin-bottom:10px;">
      <button type="submit" class="btn-save" style="flex:unset;padding:11px 28px;">🔐 Verbindung speichern</button>
  ...
  <form method="POST" id="ig-settings-form">
    <?= wcr_csrf_field() ?>
  ...
  <div class="ig-section-head">🧪 Debug</div>
  <div class="ig-debug">
    <div class="ig-debug-card">
      <div class="ig-debug-title">Aktuelle Feed-Vorschau</div>
      <p class="ig-debug-text">Zeigt die ersten 2 Einträge aus <code>/wp-json/wakecamp/v1/instagram</code>.</p>
      <pre class="ig-debug-code"><?= htmlspecialchars((string)$ig_rest_preview) ?></pre>
    </div>
    <div class="ig-debug-card">
      <div class="ig-debug-title">Hinweise</div>
      <p class="ig-debug-text">
        Änderungen an Filter, Grid, CTA und Video senden jetzt keinen Token mehr mit.
        Der Token bleibt gespeichert, bis du ihn hier oben bewusst neu setzt.
      </p>
      <p class="ig-debug-text" style="margin-top:8px;">
        Wenn der Feed hier Daten zeigt, aber <code>/insta/</code> leer bleibt, liegt der Fehler im Frontend und nicht mehr in der Verbindung.
      </p>
    </div>
  </div>

  <div class="ig-footer">
    <button type="submit" class="btn-save" style="flex:unset;padding:11px 28px;">💾 Feed-Einstellungen speichern</button>
    <span class="ig-status <?= $token_class ?>"><?= htmlspecialchars($token_status) ?></span>
  </div>
  </form>
</div>
